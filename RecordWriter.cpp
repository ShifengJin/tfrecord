...
namespace Darker {
    ...
    static const unsigned int table0_[256] = {
    ...
    static const unsigned int table1_[256] = {
    ...
    static const unsigned int table2_[256] = {
    ...
    static const unsigned int table3_[256] = {
    ...
    unsigned int RecordWriter::Extend(unsigned int crc, const char* buf, size_t size)
    {
        const unsigned char* p = reinterpret_cast<const unsigned char*>(buf);
        const unsigned char* e = p + size;
        unsigned int l = crc ^ 0xffffffffu;
...
#define STEP4                                          \
  do {                                                 \
    ...
    l = table3_[c & 0xff] ^ table2_[(c >> 8) & 0xff] ^ \
        ...
        const unsigned char* x = reinterpret_cast<const unsigned char*>(((pval + 3) >> 2) << 2);
        if (x <= e) {
...
#undef STEP1
    ...
    void RecordWriter::EncodeFixed64(char* buf, unsigned __int64 value)
    {
        memcpy(buf, &value, sizeof(value));
    }

    void RecordWriter::EncodeFixed32(char* buf, unsigned int value)
    {
        memcpy(buf, &value, sizeof(value));
    }

    unsigned int RecordWriter::DecodeFixed32(const char* ptr)
    {
        // Load the raw bytes
        unsigned int result;
    ...
    unsigned short RecordWriter::DecodeFixed16(const char* ptr)
    {
        // Load the raw bytes
    ...
    unsigned __int64 RecordWriter::DecodeFixed64(const char* ptr)
    {
        // Load the raw bytes
    ...
    RecordWriter::RecordWriter(std::string fName)
    {
    ...
    int RecordWriter::WriteRecord(StringPiece data)
    {
    ...
    inline void RecordWriter::PopulateHeader(char* header, const char* data, size_t n)
    {
        EncodeFixed64(header + 0, n);
        EncodeFixed32(header + sizeof(unsigned __int64),
            MaskedCrc(header, sizeof(unsigned __int64)));
    }

    inline void RecordWriter::PopulateFooter(char* footer, const char* data, size_t n)
    {
        EncodeFixed32(footer, MaskedCrc(data, n));
    }


    void* InitTFRecordWriter(std::string filePath) {
        RecordWriter* pInstance = new RecordWriter(filePath);
    ...
    void TFRecordWriteDataBlock(void* env, std::vector<TFRecordDataBlock>& inDataBlocks) {
        if (env == NULL) {
            return;
        }
        RecordWriter* pInstance = (RecordWriter*)env;
        if (pInstance->IsInited() == false) {
            return;
        }

        Example example;
        Features* features = example.mutable_features();
        ::google::protobuf::Map<std::string, Feature>* feature = features->mutable_feature();

        Feature feature_tmp;

        for (int i = 0; i < inDataBlocks.size(); ++i) {
            feature_tmp.Clear();
            switch (inDataBlocks[i].type)
            {
            case TYPE_BytesList: 
            {
                BytesList* bytesList = feature_tmp.mutable_bytes_list();
                bytesList->add_value(inDataBlocks[i].data, inDataBlocks[i].dataSize);
                std::string name = inDataBlocks[i].dataName;
                (*feature)[name] = feature_tmp;
            };break;
            case TYPE_FloatList:
            {
                FloatList* floatsList = feature_tmp.mutable_float_list();
                floatsList->add_values((float*)(inDataBlocks[i].data), inDataBlocks[i].dataSize / sizeof(float));
                std::string name = inDataBlocks[i].dataName;
                (*feature)[name] = feature_tmp;
            };break;
            case TYPE_Int64List:
            {
                Int64List* int64List = feature_tmp.mutable_int64_list();
                int num = inDataBlocks[i].dataSize / sizeof(__int64);
                for (int c = 0; c < num; ++c) {
                    int64List->add_value(((__int64*)inDataBlocks[i].data)[c]);
                }
                std::string name = inDataBlocks[i].dataName;
                (*feature)[name] = feature_tmp;
            };break;
        ...
        pInstance->WriteRecord(str);
    }

    void TFRecordWriterClose(void* env) {
        if (env == NULL) {
            return;
        }
        RecordWriter* pInstance = (RecordWriter*)env;
